...
try:
    ...
    VERSION = "Unknown Shotmanager version"
...
class ShotManager():
    def __init__(self):
        ...
        self.is_sandboxed = 'SOLOLINK_SANDBOX' in os.environ
    ...
    def Start(self, vehicle):
        ...
        if not self.is_sandboxed:
            ...
            self.rcSock = socket.socket( socket.AF_INET, socket.SOCK_DGRAM )
            self.rcSock.bind(('0.0.0.0', 13340))
        ...
        # Always call the EKF_OK on startup
        # sometimes the DroneKit EKF_OK callback
        # gets triggered before ShotManager Initialized
        self.ekf_callback(self.vehicle, 'ekf_ok', self.vehicle.ekf_ok)
        # set callback for when ekf changes
        self.vehicle.add_attribute_listener('ekf_ok', self.ekf_callback)
        ...
        self.vehicle.add_attribute_listener('camera_trigger', self.camera_trigger_callback)
        ...
        if len(self.currentPacket) == app_packet.SOLO_MESSAGE_HEADER_LENGTH + self.currentPacketLength:
            ...
            if self.currentPacketType == app_packet.SOLO_MESSAGE_SET_CURRENT_SHOT:
                ...
                if self.curController:
                    ...
                    logger.log("[shotManager]: location received from app: %f, %f, %f" %( lat, lon, alt ) )
            ...
            # app is trying to map a button
                ...
                if self.curController and self.currentShot == shots.APP_SHOT_FOLLOW:
                    self.curController.handleOptions(value, version=1)
            elif self.currentPacketType == app_packet.SOLO_FOLLOW_OPTIONS_V2:
                if self.curController and self.currentShot == shots.APP_SHOT_FOLLOW:
                    self.curController.handleOptions(value, version=2)
            elif self.currentPacketType == app_packet.SOLO_SHOT_OPTIONS:
                if self.curController and self.currentShot != shots.APP_SHOT_CABLECAM:
                    self.curController.handleOptions(value)
            # if it's a spline record packet (50)
            elif self.currentPacketType == app_packet.SOLO_SPLINE_RECORD:
                if self.curController and self.currentShot == shots.APP_SHOT_MULTIPOINT:
                    self.curController.enterRecordMode()

            # if it's a spline play packet (51)
            elif self.currentPacketType == app_packet.SOLO_SPLINE_PLAY:
                if self.curController and self.currentShot == shots.APP_SHOT_MULTIPOINT:
                    self.curController.enterPlayMode()

            # if it's a spline point packet (52)
            elif self.currentPacketType == app_packet.SOLO_SPLINE_POINT:
                if self.curController and self.currentShot == shots.APP_SHOT_MULTIPOINT:
                    self.curController.loadLocation(value, version=0)

            # if it's a spline seek packet (53)
            elif self.currentPacketType == app_packet.SOLO_SPLINE_SEEK:
                if self.curController and self.currentShot == shots.APP_SHOT_MULTIPOINT:
                    self.curController.handleSeek(value)

            # if it's a spline path settings (55)
            elif self.currentPacketType == app_packet.SOLO_SPLINE_PATH_SETTINGS:
                if self.curController and self.currentShot == shots.APP_SHOT_MULTIPOINT:
                    self.curController.handlePathSettings(value)

            # if it's a spline attach packet (57)
            elif self.currentPacketType == app_packet.SOLO_SPLINE_ATTACH:
                if self.curController and self.currentShot == shots.APP_SHOT_MULTIPOINT:
                    self.curController.handleAttach(value)
    ...
    def handleButtons(self):
        ...
        if self.currentShot == shots.APP_SHOT_NONE:
            if event == btn_msg.Press:
                if button == btn_msg.ButtonA or button == btn_msg.ButtonB:
                    ...
                    allowedShots = [shots.APP_SHOT_ORBIT, shots.APP_SHOT_CABLECAM, shots.APP_SHOT_MULTIPOINT]
        ...
        if button == btn_msg.ButtonCameraClick and event == btn_msg.Press:
            self.goproManager.handleRecordCommand(self.goproManager.captureMode, RECORD_COMMAND_TOGGLE)
        ...
        if self.currentShot != shot:
            ...
            if self.vehicle.armed:
                if self.vehicle.system_status == 'ACTIVE':
                    self.triggerShot(shot)
                elif self.vehicle.system_status == 'STANDBY':
                    ...
                    self.sendPacket(packet)
                elif self.vehicle.system_status == 'CRITICAL':
                    ...
                    self.sendPacket(packet)
                elif self.vehicle.system_status == 'EMERGENCY':
                    ...
                    logger.log("[enterShot]: Unexpected MAV STATUS: %s" % self.vehicle.system_status)
            else:
                logger.log("[enterShot]: Copter needs to be flying before entering shot: %s" % shots.SHOT_NAMES[shot])
                # unarmed, send an error code to the app
                packet = struct.pack('<III', app_packet.SOLO_SHOT_ERROR, 4, app_packet.SHOT_ERROR_UNARMED)
    ...
    def mode_callback(self, vehicle, name, mode):
        if mode.name != self.lastMode:
            logger.log("mode_callback:  lastMode is %s, newMode is %s"%(self.lastMode, mode.name))
            if self.currentShot != shots.APP_SHOT_NONE:
                ...
                elif mode.name == 'RTL' or mode.name == 'LAND':
    ...
    def camera_trigger_callback(self, vehicle, name, msg):
        if self.isMission:
